...
var index_default = {
  async fetch(request, env, ctx) {
    ...
    // New endpoint for newest iOS version
    if (request.method === "GET" && url.pathname === "/newest_ios") {
      ...
      try {
        ...
        // Get firmware data for target device
        const deviceData = firmwareCache[TARGET_DEVICE];
        
        if (!deviceData || !deviceData.data || !deviceData.data.firmwares) {
          return new Response(JSON.stringify({ 
            error: "No firmware data available",
            ios: "N/A",
            build: "N/A",
            size: "N/A"
          }), {
        ...
        // Get the latest firmware (first in array)
    ...
    if (request.method === "GET" && url.pathname === "/devices") {
      ...
      for (const d of deviceList) {
        let type = d.identifier.split(",")[0].replace(/\d+/g, "").replace("Watch", "Apple Watch").replace("AudioAccessory", "HomePod").replace("RealityDevice", "Vision Pro");
  ...
  async scheduled(event, env, ctx) {
    ...
    if (deviceData) {
      ...
      if (Date.now() - fetchedAt.getTime() < env.DEVICE_LIST_CACHE * 60 * 1e3) shouldFetchDevices = false;
    }
    if (shouldFetchDevices) {
      const devices = await fetch("https://api.ipsw.me/v4/devices").then((res) => res.json());
      await env.NOTIFY.put("device_list", JSON.stringify({ fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), devices }), { expirationTtl: 86400 });
      deviceData = JSON.stringify({ fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), devices });
    }
    ...
    async function fetchFirmware(deviceId) {
      ...
      const now = Date.now();
      if (entry && now - new Date(entry.fetchedAt).getTime() < env.KV_CACHE_INVALID * 60 * 1e3) {
        return entry.data;
      }
    ...
    __name(fetchFirmware, "fetchFirmware");
...
__name(fetchFirmwareAndUpdateCache, "fetchFirmwareAndUpdateCache");
async function sendEmailLambda(env, to, device, version, unsubscribeToken, messageType) {
  ...
  let emailTemplate = rawTemplate.replace("${device}", device).replace("${version}", version).replace("${unsubscribeUrl}", unsubscribeUrl);
...
__name(sendEmailLambda, "sendEmailLambda");
export {
